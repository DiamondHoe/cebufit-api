        ...
        public async Task<byte[]> GenerateExcel(List<Day> days)
        {
            ...
            try
            {
                ...
                Dictionary<Guid, double>
                    ingredientSum =
                        new Dictionary<Guid, double>(); // Dictionary to store summed quantities by base product ID
                    ...
                    foreach (var meal in day.Meals)
                    {
                        foreach (var ingredient in meal.Ingredients)
                        {
                            ...
                            if (ingredientSum.ContainsKey(ingredient.Product.Id))
                            {
                                ...
                                ingredientSum[ingredient.Product.Id] +=
                                    (double)(ingredient.Quantity ?? ingredient.Weight ?? 0);
                            }
                            else
                            {
            ...
            {
                throw new Exception("Error generating Excel file.");
            }
